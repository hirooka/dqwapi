...
public class KokoroService implements IKokoroService {
  ...
  public List<Combination> getCombinations(
    ...
    if (jobType.equals(JobType.GOD_HAND)) {
      repositoryIdentifier = UPPER_UNDERSCORE.to(LOWER_HYPHEN, dwhType.name()) + repositorySuffix + "5";
    } else {
      repositoryIdentifier = UPPER_UNDERSCORE.to(LOWER_HYPHEN, dwhType.name()) + repositorySuffix;
    }
    final List<Combination> combinations = applicationContext
    ...
    stopWatch.stop();
    log.info("{} results, {} ms",
            combinations.size(), String.format("%,d", stopWatch.getLastTaskTimeMillis())
    );
